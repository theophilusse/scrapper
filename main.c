...
t_html_node         *html_new_node(char *tag, t_html_node *parent, char **end)
{
    ...
    if (!node->is_inline)
        while ((inner_node = html_new_node(trail, node, &nexttag))) // ORIGINAL
        {
...
char            *buffer_param_get_var_buf(t_buf *buf, char *varname)
{
    ...
    while (++i < buf->size)
    {
        ...
        if (strncmp(param->name, varname, STRING_SIZE) == 0)
...
struct s_http_content   http_new_content(char *type, t_buf *data)
{
    ...
    strncpy(content.type, type, STRING_SIZE);
...
struct s_http_request   http_new_request(char *url, char *method, t_buf *param, t_buf *header, t_http_content *content)
{
    ...
    if (!method || !url)
        return (request);
    strncpy(request.method, method, STRING_SIZE);
...
char            *http_send_request(t_http_request *request, uint *recv_length)
{
    ...
    printf("Route [%s]\n", route);
    buffer = NULL;
    buffer = string_stradd(buffer, request->method);
    buffer = string_stradd(buffer, " ");
    buffer = string_stradd(buffer, route);
    FREE(route);
    i = -1;
    printf("Param :\n");
    ...
    buffer = string_stradd(buffer, " HTTP/1.1\r\n");
    printf("Headers :\n");
    if (!url_is_ipv4(host))
    {
        buffer = string_stradd(buffer, "Host: ");
        buffer = string_stradd(buffer, host);
        buffer = string_stradd(buffer, "\r\n");
    }
    FREE(host);
    ...
    {
        buffer = string_stradd(buffer, "Content-Type: ");
        buffer = string_stradd(buffer, request->content.type);
        buffer = string_stradd(buffer, "\r\n");
    }
    i = -1;
    ...
    if (strlen(request->content.type) != 0)
    {
        ...
        {
            //buffer = string_stradd(buffer, request->content.data.buf, request->content.data.size)
            buffer = string_stradd(buffer, request->content.data.buf);
        }
    }
    buffer = string_stradd(buffer, "\r\n\r\n\r\n");
    ///printf("%s\n", buffer);
    ret = net_send_recv(&con, buffer, NULL, recv_length);
    FREE(buffer);
    net_disconnect(&con);
    return (ret);
}
...
struct s_http_response  web_get_page_request(char *url, t_http_request *out)
{
    struct s_buf            header;
    struct s_http_request   request;
    struct s_http_response  response;
...
t_buf_param             *buffer_copy_param(t_buf_param *param)
{
    ...
    if (!(copy = ALLOC(sizeof(struct s_buf_param))))
        return (NULL);
    strncpy(copy->name, param->name, STRING_SIZE);
...
struct s_http_response   http_response_import_xml(t_html_node *node)
{
    ...
    if (!(ptr = buffer_param_get_var_buf(&resnode->param, "version")))
        return (response);
    strncpy(response.http_version, ptr, STRING_SIZE);
...
struct s_http_request   http_request_import_xml(t_html_node *node)
{
    ...
    if (!(ptr = buffer_param_get_var_buf(&reqnode->param, "method")))
        return (request);
    strncpy(request.method, ptr, STRING_SIZE);
...
t_web_node          *web_new_node(char *url, t_web_node *parent, uint maxdepth)
{
    ...
    page = web_get_page_request(url, &node->request);
    ...
    while (++i < list.size)
    {
        ...
        while (++j < tag->param.size)
        {
            ...
            if (strncasecmp(param->name, "href", STRING_SIZE) == 0)
            {
                ...
                if ((child = web_new_node(full_url, node, maxdepth - 1)))
                {
                    DEBUG //
...
void            test_import_export(void)
{
    ...
    */
    ...
    if (!(root = web_new_node("http://info.cern.ch/", NULL, 1)))
        printf("Error\n");
    char *xml;
